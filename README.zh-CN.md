...
Releases 里也提供一个 COM3D2（非 2.5）专用构建：`COM3D2.ExclusiveMaidNTRBlock.Plugin.dll`。这个 DLL 是给 COM3D2 用的，不要和 COM3D2.5 版混用。
...
1. COM3D2.5 从 Releases 页面下载 `COM3D25.ExclusiveMaidNTRBlock.Plugin.dll`；COM3D2（非 2.5）下载 `COM3D2.ExclusiveMaidNTRBlock.Plugin.dll`
...
各功能模块会单独注册 patch。如果某个 DLC 或模式在当前游戏里缺少对应类型/方法，插件会跳过那个模块，并尽量让其它可用模块继续生效。
